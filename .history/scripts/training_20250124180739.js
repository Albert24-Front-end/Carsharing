...
function renderCars(cars) {
    ...
    carsContainer.innerHTML = ''; // Очистить контейнер перед рендерингом
...
function filteredCars(cars) {
    ...
    filterItems.forEach((filter) => {
        filter.addEventListener('click', () => {
            ...
            const filterText = filter.textContent.toLowerCase();r.brand.toLowerCase().includes(filterText
            const filteredCars = filterText === 'все марки' ? cars : cars.filter((car) => ca));
